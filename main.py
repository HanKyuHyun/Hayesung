        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
                for _, row in final_df.iterrows():
                    ...
                    draw.text((190, 775), str(row['수급자명']), fill="black", font=f_name)
                    draw.text((370, 775), str(row['인정관리번호']), fill="black", font=f_main)
                    draw.text((750, 750), date_range, fill="black", font=f_date)
                    
                    # 2. 왼쪽 금액 항목 (본인부담, 공단부담, 급여계 순서)
                    # 위치를 훨씬 아래로(1300번대 이상으로) 조정
                    draw.text((730, 890), f"{own_amt:,}", fill="black", font=f_main) 
                    draw.text((730, 970), f"{pub_amt:,}", fill="black", font=f_main) 
                    draw.text((730, 1040), f"{total_amt:,}", fill="black", font=f_main) 
