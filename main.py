...
def draw_invoice(row, date_range, publish_date_str):
   ...
   # 1. 인적사항: 더 왼쪽으로, 더 아래로 (Y축 760 -> 785)
    ...
    draw.text((900, 888), f"₩{own_amt:,}", fill="black", font=f_main,anchor="ra") 
    draw.text((900, 960), f"₩{pub_amt:,}", fill="black", font=f_main,anchor="ra") 
    draw.text((900, 1030), f"₩{total_amt:,}", fill="black", font=f_main,anchor="ra") 
    
    # 3. 오른쪽 '금액산정내역': 오른쪽 선 침범 방지를 위해 왼쪽으로 많이 당김 (1350->1250)
    draw.text((1650, 915), f"₩{total_amt:,}", fill="black", font=f_main,anchor="ra") 
    draw.text((1650, 1010), f"₩{own_amt:,}", fill="black", font=f_main,anchor="ra")
